...
export default function SyllabusPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {/* Topbar — title + add only, positioned below mobile hamburger */}
      <div className="topbar top-[52px] md:top-0">
        ...
        {grouped.map(course => {
          ...
          return (
            <div key={course.id} className="card overflow-hidden">
              ...
              >
                <span className="text-xs font-bold text-neutral-500 dark:text-neutral-500 w-5 flex-shrink-0">{course.number}.</span>
                <span className="text-sm font-semibold text-neutral-900 dark:text-white flex-1 min-w-0 truncate">{course.name}</span>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <div className="hidden sm:block w-20 h-1.5 bg-neutral-200 dark:bg-neutral-700 rounded-full overflow-hidden">
                    <div
                      className="h-full rounded-full bg-neutral-900 dark:bg-brand-500"
                      style={{ width: `${coursePct}%` }}
                    />
                  </div>
                  <span className="text-xs text-neutral-400 w-7 text-right">{coursePct}%</span>
                ...
                <div className="divide-y divide-neutral-50 dark:divide-neutral-800/60">
                  {course.topics.map(t => (
                    <div key={t.id} className="flex items-center gap-2 px-3 sm:px-4 py-2.5 hover:bg-neutral-50 dark:hover:bg-neutral-900/40 transition-colors">
                      ...
                      <span className="text-xs text-neutral-700 dark:text-neutral-300 flex-1 min-w-0 pr-1">{t.name}</span>
                      <select
                        value={t.status}
                        onChange={e => updateStatus(t.id, e.target.value)}
                        className="text-[11px] px-1.5 py-0.5 rounded-lg border border-neutral-200 dark:border-neutral-700
                                   bg-transparent text-neutral-600 dark:text-neutral-400 cursor-pointer flex-shrink-0
